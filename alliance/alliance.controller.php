<?php
if(!class_exists('allianceControllerAPI'))
{
	require _XE_PATH_.'modules/alliance/alliance.controller.api.php';
}
...
class allianceController extends allianceControllerAPI
{
	...
	{
		// 비정상적인 접근 차단
		if($this->module_info->module != 'alliance')
		{
	 ...
	 * @brief 부모 사이트 등록
	 */
	public function insertParentSite($domain, $title, $accept = false)
	{
		return self::_insertSite($domain, $title, 'P', $accept);
	}

	/**
	 * @brief 자식 사이트 등록
	 */
	public function insertChildSite($domain, $title, $accept = false)
	{
		return self::_insertSite($domain, $title, 'C', $accept);
	}

	/**
	 * @brief 친구 사이트 등록
	 */
	public function insertFriendSite($domain, $title, $accept = false)
	{
		return self::_insertSite($domain, $title, 'F', $accept);
	}

	/**
	 * @brief 사이트 등록
	 */
	private function _insertSite($domain, $title, $type, $accept = false)
	{
		if(!$domain)
		{
			return;
	...
	function triggerAfterLogin(&$member_info) {
		/**
		 * @TODO : 다른 도메인 간에도 로그인 동기화를 가능하게 할 수 있는 방법이 필요함 (jsonp)
		 */
		return new Object();
